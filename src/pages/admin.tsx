...
import {
  ...
  FileArchive, Download, Loader2, Flag,
} from "lucide-react";
...
type Tab = "overview" | "deposits" | "withdrawals" | "referrals" | "ads" | "users" | "reviews";
...
const EMPTY: AdminData = { deposits: [], withdrawals: [], tasks: [], campaigns: [], settings: {}, profiles: [], reports: [], bans: [], reviewRequests: [] };
...
function useAdminData() {
  ...
  useEffect(() => {
    ...
    cachedQuery("admin:v1", 60_000, async (): Promise<AdminData | null> => {
      ...
      if (!sb) return null;
      const [d, w, t, c, s, p, r, b, rv] = await Promise.all([
      ...
      if (d.error || w.error || t.error || c.error || s.error || p.error || r.error || b.error || rv.error) return null;
...
export default function AdminPage() {
  ...
  const banUser = (handle: string) => {
    queueWrite("bans", { handle, until: new Date(Date.now() + 7 * 86400_000).toISOString(), reason: "Banned by admin" }, "handle");
    refresh();
  };
    ...
    <div className="max-w-[1200px] mx-auto space-y-6">
      ...
      {loading ? (
          ...
          {mostReported ? (
            <div className="rounded-2xl border border-rose-400/20 bg-rose-500/[0.05] p-5 flex items-center gap-4">
              ...
              {activeBanHandles.has(mostReported[0]) ? (
                ...
                <button onClick={() => banUser(mostReported[0])} className="btn-ghost flex items-center gap-1.5 shrink-0 !text-rose-300">
        ...
        <div className="space-y-4">
          ...
          <AdminTable
            head={["User", "Tier", "Premium", "Reports", "Status", ""]}
            rows={data.profiles
              ...
              .map((p) => {
                ...
                return [
                  ...
                  banned ? (
                    <span key="s" className="px-2 py-0.5 rounded-md border border-rose-400/25 bg-rose-500/15 text-rose-300 text-[10px] font-bold uppercase flex items-center gap-1 w-fit">
                  ...
                  banned ? (
                    <button key="x" onClick={() => unbanUser(p.handle)} className="text-[11px] font-bold text-emerald-300 hover:text-emerald-200 flex items-center gap-1">
                      <Unlock className="w-3.5 h-3.5" /> Unban
                    </button>
                  ) : (
                    <button key="x" onClick={() => banUser(p.handle)} className="text-[11px] font-bold text-rose-300 hover:text-rose-200 flex items-center gap-1">
